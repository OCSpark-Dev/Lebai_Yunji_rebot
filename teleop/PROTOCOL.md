...
- 连续有效的非零运动命令期间，服务端累计命令时长和速度得到可观测预期位移；达到阈值后若仍没有与当前笛卡尔命令方向一致的 TCP 进度，也没有位移与反馈速度相互一致的关节进度，则撤销租约并停止。随机微抖、正交/反向噪声不算 TCP 进度，连续换向不得重置无进度窗口；该软件检测仍须用真机标定阈值和停机行为。
- 桥接器不得把连续命令无界堆入乐白异步运动缓冲。
- UI 松手时立即发送 `motion.stop`，不能等待下一个定时周期；协议仍允许显式全零 `motion.cartesian_velocity` 作为额外停止冗余。
...
在硬件模式下，安全桥不等待普通 `pylebai` 后端锁，而是默认通过控制器 `3031` 端口的独立 HTTP JSON-RPC 连接请求 `stop_move`，默认软件 deadline 为 200 ms。超时时应记录发生在 `connect`、`request`、`first_byte` 或 `body` 阶段，以及总耗时和配置 deadline。该 deadline 只约束桥接器的停止请求等待时间；Windows、Python、网络和机器人控制器都不是硬实时链路，不能据此保证 200 ms 内物理停止。普通 `pylebai` RPC 也可能延迟，因此有限命令持续时间、独立 stop、物理急停和现场监护必须同时存在。
...
- 限幅后的姿态角速度范数低于 `1e-3 rad/s` 时视为传感器死区：服务端仍 ACK、续租和刷新有效命令时间，但 `duration_ms=0`，不调用 `speedl` 或 `stop_move`。这个例外只适用于 `pose.sample`；普通笛卡尔速度消息的显式全零命令仍下发 `speedl(0)` 作为停止冗余。
